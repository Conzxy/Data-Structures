...
```
相比next数组，nextval增加了当前字符和前缀后一字符的判定，若相同，则next值替换前缀后一字符的next值成为新的nextval值，不同，则照旧。因为当前字符和前缀后一字符相同的话，再度匹配是肯定失败的，因此没有必要，直接回溯到前缀即可。（若前后缀没有相同的字符，则认为前缀长度为0，即比较对象为第一个字符）
### nextval数组
#### e.g
| j    | 0|1|2|3|4|5|6|7|8|
| :--- |:----:|:----:|:----:|:----:|:----:|:----:|:----:|:----:|:----:|
...
next转nextval只要看看要回溯的前缀后一字符和自己是不是相同就行了，若相同，就等于前缀后一字符的next值，否则，照旧。
